...
export function calculateShipping(
    ...
    let prix = getDistanceCout(distance)
    prix *= getMajorationMultiplier(weight)
    if (type == 'express') {prix *= 2}
    return prix
}

function getDistanceCout(distance: number): number {
    if (distance <0) {throw Error("Invalid distance")}

    if (distance <51) {return 10}
    else if (51 <= distance && distance <= 500) {return 25}
    else {return 50}
}

function getMajorationMultiplier(poids: number): number {
    if (poids <= 0 || 50 < poids) {throw Error("Invalid weight")}

    if (poids < 10) {return 1}
    else {return 1.5}
}